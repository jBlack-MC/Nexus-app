...
| Project planning | Tasks support due dates, priority, status, labels, and checklists; dashboard surfaces attention sections. | task UI/ViewModels and `DashboardScreen.kt` |
...
Insert four real running-app screenshots: Dashboard, project creation, task completion, and Settings. Add the unlisted narrated demo link after recording. The narration must cover registration, login, project/task creation, completion, Settings, secure session storage at a high level, and the matching PostgreSQL rows. See `DEMO_SCRIPT.md` for the sequence.
