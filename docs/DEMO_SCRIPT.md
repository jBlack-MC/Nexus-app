...
3. **Log in (0:45-1:00).** Log out, then log back in. “The bearer token is attached automatically to protected API calls.”
...
5. **Create and complete tasks (1:25-2:05).** Add two tasks, giving one a due date/priority and the other a checklist. Mark one complete. “Task metadata makes urgency and progress visible, and the dashboard separates overdue, due-today, and upcoming work.”
...
7. **Data evidence (2:30-3:00).** Show the auth store/session record and the PostgreSQL `users`, `projects`, and `tasks` rows for the demonstrated account. Do not expose a real token or password in the recording.
