...
data class GamificationStats(
    ...
    companion object {
        fun from(habits: List<Habit>, today: String): GamificationStats {
        ...
        private fun streakEndingAt(dates: List<String>, end: String): Int {
            var streak = 0
            var cursor = end
        ...
        private fun shiftDate(date: String, amount: Int): String {
            val parts = date.split('-').map(String::toInt)
            val calendar = java.util.Calendar.getInstance().apply {
                set(parts[0], parts[1] - 1, parts[2], 0, 0, 0)
                add(java.util.Calendar.DAY_OF_MONTH, amount)
            }
