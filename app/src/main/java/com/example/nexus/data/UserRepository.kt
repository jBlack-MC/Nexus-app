...
open class UserRepository(
    ...
    open suspend fun register(request: RegisterRequest): AuthResponse = apiService?.register(request) ?: throw IllegalStateException("No service")
    open suspend fun getProfile(): UserProfile = apiService?.getProfile() ?: throw IllegalStateException("No service")
    open suspend fun updateProfile(request: UpdateProfileRequest): UserProfile = apiService?.updateProfile(request) ?: throw IllegalStateException("No service")
    open suspend fun changePassword(request: ChangePasswordRequest) { apiService?.changePassword(request) }
    open suspend fun deleteAccount() { apiService?.deleteAccount() }
}
